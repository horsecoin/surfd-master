...
rvec shows total number of SURF satoshis created since genesis for:
...
- Missed blocks not modeled (lowers SURF supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise SURF supply, very small)
- Creation / destruction of SURF used to back SBD not modeled (moves SURF supply in direction opposite to changes in dollar value of 1 SURF, large)
- Interest paid to SBD not modeled (raises SURF supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of SURF not modeled (lowers SURF supply, unknown but likely small)
...
/*
#define SURF_GENESIS_TIME                    (fc::time_point_sec(1458835200))
#define SURF_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
...
//   {
//      uint32_t vesting_factor = (block_num < SURF_START_VESTING_BLOCK) ? 0 : 9;
//
//      share_type curate_reward   = calc_percent_reward_per_block< SURF_CURATE_APR_PERCENT >( current_supply );
//      reward_delta[ CURATE_OFF ] = std::max( curate_reward, SURF_MIN_CURATE_REWARD.amount );
//      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;
//
//      share_type content_reward  = calc_percent_reward_per_block< SURF_CONTENT_APR_PERCENT >( current_supply );
//      reward_delta[ CONTENT_OFF ] = std::max( content_reward, SURF_MIN_CONTENT_REWARD.amount );
//      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;
//
//      share_type producer_reward = calc_percent_reward_per_block< SURF_PRODUCER_APR_PERCENT >( current_supply );
//      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, SURF_MIN_PRODUCER_REWARD.amount );
...
//
//   for( uint32_t b=1; b<10*SURF_BLOCKS_PER_YEAR; b++ )
//   {
