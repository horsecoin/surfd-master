...
target_link_libraries( {plugin_provider}_{plugin_name} surf_app surf_chain surf_protocol )
...
{{
   public:
      {plugin_name}_api( const surf::app::api_context& ctx );
...
class {plugin_name}_plugin : public surf::app::plugin
{{
   public:
      {plugin_name}_plugin( surf::app::application* app );
...
{{
   public:
      {plugin_name}_api_impl( surf::app::application& _app );
...
{plugin_name}_api_impl::{plugin_name}_api_impl( surf::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const surf::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( surf::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( surf::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( surf::app::application* app )
   : plugin(app)
{{
...
SURF_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (surf for plugins developed by Surf)")
