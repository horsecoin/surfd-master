...
#include <surf/blockchain_statistics/blockchain_statistics_plugin.hpp>
...
{
   public:
      blockchain_statistics_api( const surf::app::api_context& ctx );
...
FC_REFLECT( surf::blockchain_statistics::statistics,
   (blocks)
   (bandwidth)
   (operations)
   (transactions)
...
FC_API( surf::blockchain_statistics::blockchain_statistics_api,
   (get_stats_for_time)
