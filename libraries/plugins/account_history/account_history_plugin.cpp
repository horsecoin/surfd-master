...
#define SURF_NAMESPACE_PREFIX "surf::protocol::"

namespace surf { namespace account_history {
...
void account_history_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   if( options.count( "history-whitelist-ops" ) )
   {
      ...
      for( auto& arg : options.at( "history-whitelist-ops" ).as< vector< string > >() )
      {
         ...
         {
            if( op.size() )
               my->_op_list.insert( SURF_NAMESPACE_PREFIX + op );
         }
      }
...
SURF_DEFINE_PLUGIN( account_history, surf::account_history::account_history_plugin )
