...
namespace surf { namespace protocol {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, SURF_SYMBOL ), "Account creation fee must be SURF" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, SURF_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= SURF_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= SURF_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= SURF_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( max_accepted_payout.symbol == SURF_SYMBOL, "Max accepted payout must be in SURF_SYMBOL" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= SURF_100_PERCENT, "Weight is not a valid percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of Surf Power (STMP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < SURF_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, SURF_SYMBOL ), "Amount must be SURF" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, SURF_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= SURF_100_PERCENT, "Percent must be valid surf percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( fee >= asset( 0, SURF_SYMBOL ), "Fee cannot be negative" );
      props.validate();
   }
