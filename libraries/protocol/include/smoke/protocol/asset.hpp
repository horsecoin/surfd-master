...
namespace surf { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = SURF_SYMBOL )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const surf::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  surf::protocol::asset& vo ) { vo = surf::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( surf::protocol::asset, (amount)(symbol) )
FC_REFLECT( surf::protocol::price, (base)(quote) )
