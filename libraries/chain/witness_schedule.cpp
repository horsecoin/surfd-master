...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( SURF_MAX_WITNESSES );
...
//   const auto& gprops = db.get_dynamic_global_properties();
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < SURF_MAX_WITNESSES;
        ++sitr )
   {
   ...
   size_t expected_active_witnesses = std::min( size_t(SURF_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("SURF_MAX_WITNESSES",SURF_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < SURF_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % SURF_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
