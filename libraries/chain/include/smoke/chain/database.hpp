...
#include <surf/chain/node_property_object.hpp>
#include <surf/chain/fork_database.hpp>
...
namespace surf { namespace chain {
   ...
   using surf::protocol::signed_transaction;
   using surf::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
          ...
          */
         void open( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t initial_supply = SURF_INIT_SUPPLY, uint64_t shared_file_size = 0, uint32_t chainbase_flags = 0 );
          ...
          *
          * Passing slot_num == 0 returns SURF_NULL_WITNESS
          */
         ...
         asset create_vesting( const account_object& to_account, asset surf, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& total_payout_value, const asset& curator_surf_value, const asset& beneficiary_value );
         ...
         void init_genesis(uint64_t initial_supply = SURF_INIT_SUPPLY);
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ SURF_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ SURF_NUM_HARDFORKS + 1 ];
