...
using surf::protocol::block_id_type;
using surf::protocol::transaction_id_type;
using surf::protocol::chain_id_type;
using surf::protocol::account_name_type;
...
{
   class variant;
   inline void to_variant( const surf::chain::shared_string& s, variant& var )
   {
      var = fc::string( surf::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, surf::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( surf::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const surf::chain::buffer_type& raw, T& v, uint32_t depth = 0 )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v, depth );
      }

      template< typename T > inline T unpack( const surf::chain::buffer_type& raw, uint32_t depth = 0 )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v, depth );
...
FC_REFLECT_ENUM( surf::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( surf::chain::shared_string )
FC_REFLECT_TYPENAME( surf::chain::buffer_type )

FC_REFLECT_ENUM( surf::chain::bandwidth_type, (post)(forum)(market) )
