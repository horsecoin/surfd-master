...
{
   FC_ASSERT( permlink.size() > SURF_MIN_PERMLINK_LENGTH && permlink.size() < SURF_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   FC_ASSERT( o.url.size() <= SURF_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   FC_ASSERT( o.props.account_creation_fee.symbol == SURF_SYMBOL );

//   #pragma message( "TODO: This needs to be part of HF 20 and moved to validate if not triggered in previous blocks" )
   if( _db.is_producing() )
   {
      FC_ASSERT( o.props.maximum_block_size <= SURF_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
{
#ifndef IS_TEST_NET
   FC_ASSERT( o.creator == SURF_SURF_ACCOUNT, "only surf account can create new account this period." );
#endif
...
//   const witness_schedule_object& wso = _db.get_witness_schedule_object();
   FC_ASSERT( o.fee >= asset( SURF_MIN_ACCOUNT_CREATION_FEE, SURF_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
              ("f", asset( SURF_MIN_ACCOUNT_CREATION_FEE, SURF_SYMBOL ) )
              ("p", o.fee) );
#endif
...
void account_create_with_delegation_evaluator::do_apply( const account_create_with_delegation_operation& o )
{
   ...
   if( o.delegation.amount > 0 )
   {
      _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + SURF_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   FC_ASSERT( o.account != SURF_TEMP_ACCOUNT, "Cannot update temp account." );
   ...
   {
#ifndef IS_TEST_NET
      FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > SURF_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( comment.parent_author != SURF_ROOT_POST_PARENT )
   {
...
{ try {
   ...
   if( o.parent_author != SURF_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      FC_ASSERT( parent->depth < SURF_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",SURF_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      if( o.parent_author == SURF_ROOT_POST_PARENT )
          FC_ASSERT( ( now - auth.last_root_post ) > SURF_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
      else
          FC_ASSERT( (now - auth.last_post) > SURF_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );


      uint16_t reward_weight = SURF_100_PERCENT;
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         com.cashout_time = com.created + SURF_CASHOUT_WINDOW_SECONDS;
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, SURF_SYMBOL) >= o.amount, "Account does not have sufficient SURF for transfer." );
   _db.adjust_balance( from_account, -o.amount );
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   {
      int vesting_withdraw_intervals = SURF_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(SURF_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < SURF_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= SURF_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( SURF_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= SURF_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      FC_ASSERT( voter.witnesses_voted_for < SURF_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
...
{ try {
   ...
   FC_ASSERT( elapsed_seconds >= SURF_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (SURF_100_PERCENT * elapsed_seconds) / SURF_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(SURF_100_PERCENT) );
   FC_ASSERT( current_power > 0, "Account currently does not have voting power." );

   int64_t  abs_weight    = abs(o.weight);
   int64_t  used_power    = (current_power * abs_weight) / SURF_100_PERCENT;

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // used_power = (current_power * abs_weight / SURF_100_PERCENT) * (reserve / max_vote_denom)
   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * SURF_VOTE_REGENERATION_SECONDS / (60*60*24);
   ...
   int64_t abs_rshares    = ((uint128_t(voter.effective_vesting_shares().amount.value) * used_power) / (SURF_100_PERCENT)).to_uint64();

   FC_ASSERT( ((abs_rshares > SURF_VOTE_DUST_THRESHOLD) || (o.weight == 0)),
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         FC_ASSERT( _db.head_block_time() < comment.cashout_time - SURF_UPVOTE_LOCKOUT, "Cannot increase payout within last twelve hours before payout." );
      }
...
//            const uint128_t two_s = 2 * util::get_content_constant_s();
            ...
            uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(SURF_REVERSE_AUCTION_WINDOW_SECONDS) );

            w *= delta_t;
            w /= SURF_REVERSE_AUCTION_WINDOW_SECONDS;
   ...
   {
      FC_ASSERT( itr->num_changes < SURF_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + SURF_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   FC_ASSERT( _db.head_block_time() - account.last_account_recovery > SURF_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + SURF_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void decline_voting_rights_evaluator::do_apply( const decline_voting_rights_operation& o )
{
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + SURF_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount * 10, SURF_SYMBOL ) * gpo.get_vesting_share_price();
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      _db.create< vesting_delegation_expiration_object >( [&]( vesting_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + SURF_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
