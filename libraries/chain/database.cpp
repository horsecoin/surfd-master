...
#include <surf/chain/db_with.hpp>
#include <surf/chain/evaluator_registry.hpp>
...
#include <surf/chain/witness_schedule.hpp>

#include <surf/chain/util/asset.hpp>
#include <surf/chain/util/reward.hpp>
#include <surf/chain/util/uint256.hpp>
#include <surf/chain/util/reward.hpp>
...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      open( data_dir, shared_mem_dir, SURF_INIT_SUPPLY, shared_file_size, chainbase::database::read_write );
      _fork_db.reset();    // override effect of _fork_db.start_block() call in open()

      auto start = fc::time_point::now();
      SURF_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(SURF_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //SURF_MAX_BLOCK_SIZE;
   ...
   if( witness.running_version != SURF_BLOCKCHAIN_VERSION )
      pending_block.extensions.insert( block_header_extensions( SURF_BLOCKCHAIN_VERSION ) );

   const auto& hfp = get_hardfork_property_object();

   if( hfp.current_hardfork_version < SURF_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
   ...
   else if( hfp.current_hardfork_version == SURF_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
      && witness.hardfork_version_vote > SURF_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
   {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= SURF_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      SURF_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   SURF_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   SURF_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   SURF_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   SURF_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   SURF_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   SURF_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = SURF_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / SURF_BLOCK_INTERVAL + 1;
}

/**
 * @param to_account - the account to receive the new vesting shares
 * @param surf - SURF to be converted to vesting shares
 */
asset database::create_vesting( const account_object& to_account, asset surf, bool to_reward_balance )
{
   try
   {
       ...
       */
      asset new_vesting = surf * ( to_reward_balance ? cprops.get_reward_vesting_share_price() : cprops.get_vesting_share_price() );
   ...
   FC_CAPTURE_AND_RETHROW( (to_account.name)(surf) )
}

void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, SURF_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != SURF_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= SURF_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = SURF_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = SURF_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
{
   if( head_block_num() >= SURF_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( SURF_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, SURF_SYMBOL ), true );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
  try
  {
     ...
     if( comment.net_rshares > 0 )
     {
        ...
        if( reward_tokens > 0 )
        {
           share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / SURF_100_PERCENT ).to_uint64();
           ...
           {
              auto benefactor_tokens = ( author_tokens * b.weight ) / SURF_100_PERCENT;
...
//   ctx.current_surf_price = get_feed_history().current_median_history;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      {
          fc::microseconds decay_rate = SURF_RECENT_RSHARES_DECAY_RATE;
...
void database::process_funds()
{
  ...
  int64_t start_inflation_rate = int64_t( SURF_INFLATION_RATE_START_PERCENT );
  int64_t inflation_rate_adjustment = int64_t( block_num / SURF_INFLATION_NARROWING_PERIOD );
  int64_t inflation_rate_floor = int64_t( SURF_INFLATION_RATE_STOP_PERCENT );
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / SURF_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, SURF_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more SURF than has been allocated through inflation
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + SURF_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, SURF_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < SURF_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = SURF_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(SURF_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = SURF_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SURF_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = SURF_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SURF_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
         auth.posting = authority();
         auth.posting.weight_threshold = 1;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = SURF_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SURF_TEMP_ACCOUNT;
      ...
      for( int i = 0; i < SURF_MAX_WITNESSES; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = SURF_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
#ifdef IS_TEST_NET
            a.balance  = asset( i ? 0 : init_supply, SURF_SYMBOL );
#else
            a.balance  = asset( i ? 0 : 1000, SURF_SYMBOL ); // 1 SURF to powerup surf account
#endif
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = SURF_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = SURF_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      create< account_object >( [&]( account_object& a )
        {
            a.name = SURF_SURF_ACCOUNT;
            a.memo_key = init_public_key;
            a.balance  = asset( init_supply - 1000, SURF_SYMBOL );
        } );

      create< account_authority_object >( [&]( account_authority_object& auth )
         {
             auth.account = SURF_SURF_ACCOUNT;
             auth.owner.add_authority( init_public_key, 1 );
             auth.owner.weight_threshold = 1;
             auth.active  = auth.owner;
             auth.posting = auth.active;
      ...
      {
         p.current_witness = SURF_INIT_MINER_NAME;
         p.time = SURF_GENESIS_TIME;
      ...
      {
         hpo.processed_hardforks.push_back( SURF_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = SURF_INIT_MINER_NAME;

         wso.max_voted_witnesses = SURF_MAX_VOTED_WITNESSES;
         wso.max_miner_witnesses = SURF_MAX_MINER_WITNESSES;
         wso.max_runner_witnesses = SURF_MAX_RUNNER_WITNESSES;
      } );
...
//         test_op.required_auths.insert( SURF_INIT_MINER_NAME );
...
//#endif
      ...
      auto post_rf = create< reward_fund_object >( [&]( reward_fund_object& rfo )
         {
             rfo.name = SURF_POST_REWARD_FUND_NAME;
             rfo.last_update = head_block_time();
...
//      get_changed_ids( ids );
//      SURF_TRY_NOTIFY( changed_objects, ids )*/
//      /*
//      if( _undo_db.enabled() )
//      {
...
//         }
//         SURF_TRY_NOTIFY( changed_objects, changed_ids )
//      }
//      */
//   }
//   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( block_size < SURF_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",SURF_MIN_BLOCK_SIZE)
      );
   }
...
{ try {
   ...
   const chain_id_type& chain_id = SURF_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, SURF_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         SURF_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      SURF_ASSERT( trx.expiration <= now + fc::seconds(SURF_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",SURF_MAX_TIME_UNTIL_EXPIRATION));

      SURF_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      SURF_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            modify( witness_missed, [&]( witness_object& w )
            {
               ...
               if( head_block_num() - w.last_confirmed_block_num  > SURF_BLOCKS_PER_DAY )
               {
   ...
   {
      SURF_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < SURF_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",SURF_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < SURF_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > SURF_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - SURF_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( SURF_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((SURF_100_PERCENT - SURF_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / SURF_100_PERCENT);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < SURF_BLOCKS_PER_DAY*7 )
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( SURF_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );

   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= SURF_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("SURF_NUM_HARDFORKS",SURF_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= SURF_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( SURF_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ SURF_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         if( hardforks.last_hardfork < SURF_NUM_HARDFORKS ) {
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= SURF_NUM_HARDFORKS; i++ )
   {
  ...
  try
  {
     ...
     for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
     {
        ...
        total_vsf_votes += ( itr->proxy == SURF_PROXY_TO_SELF_ACCOUNT ?
                             ...
                             ( SURF_MAX_PROXY_RECURSION_DEPTH > 0 ?
                               itr->proxied_vsf_votes[SURF_MAX_PROXY_RECURSION_DEPTH - 1] :
     ...
     for( auto itr = escrow_idx.begin(); itr != escrow_idx.end(); ++itr )
     {
        ...
        if( itr->pending_fee.symbol == SURF_SYMBOL )
           ...
           FC_ASSERT( false, "found escrow pending fee that is not SBD or SURF" );
     }
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / SURF_VESTING_WITHDRAW_INTERVALS, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < SURF_MAX_PROXY_RECURSION_DEPTH; ++i )
...
void database::retally_comment_children()
{
   ...
   for( auto itr = cidx.begin(); itr != cidx.end(); ++itr )
   {
      if( itr->parent_author != SURF_ROOT_POST_PARENT )
      {
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != SURF_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != SURF_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != SURF_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
