...
of a running Surf Debug Node. The Debug Node is a plugin for Surf that allows realtime
...
To use the script include `from surfdebugnode import DebugNode`
...
``` Python
debug_node = DebugNode( str( surfd ), str( data_dir ) )
with debug_node:
...
specify the surfd binary location and a populated data directory. For most configurations
this will be `programs/surfd/surfd` and `witness_node_data_dir` respectively, from the
git root directory for Surf.
