...
class DebugNode( object ):
   """ Wraps the surfd debug node plugin for easier automated testing of the Surf Network"""

   def __init__( self, surfd, data_dir, args='', plugins=[], apis=[], surfd_out=None, surfd_err=None ):
      """ Creates a surfd debug node.
      ...
      args:
         ...
         data_dir -- The string path to an existing surfd data directory which will be used to pull blocks from.
         args -- Other string args to pass to surfd.
         ...
         surfd_stdout -- A stream for surfd's stdout. Default is to pipe to /dev/null
         surfd_stderr -- A stream for surfd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._surfd_bin = None
      self._surfd_lock = None
      self._surfd_process = None
      self._temp_data_dir = None

      self._surfd_bin = Path( surfd )
      if( not self._surfd_bin.exists() ):
         raise ValueError( 'surfd does not exist' )
      if( not self._surfd_bin.is_file() ):
         raise ValueError( 'surfd is not a file' )
      ...
      if( surfd_out != None ):
         self.surfd_out = surfd_out
      else:
         self.surfd_out = self._FNULL

      if( surfd_err != None ):
         self.surfd_err = surfd_err
      else:
         self.surfd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._surfd_lock = Lock()


   def __enter__( self ):
      self._surfd_lock.acquire()
      ...
      surfd = [ str( self._surfd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      surfd.extend( self._args )

      self._surfd_process = Popen( surfd, stdout=self.surfd_out, stderr=self.surfd_err )
      self._surfd_process.poll()
      sleep( 5 )
      if( not self._surfd_process.returncode ):
         self._rpc = SurfNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "surfd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._surfd_process != None ):
         self._surfd_process.poll()

         if( not self._surfd_process.returncode ):
            self._surfd_process.send_signal( SIGINT )

            sleep( 7 )
            self._surfd_process.poll()

            if( not self._surfd_process.returncode ):
               self._surfd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._surfd_process.poll()

               if( self._surfd_process.returncode ):
                  loggin.error( 'surfd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._surfd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._surfd_lock.release()
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is SURF_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      ...
      debug_node = DebugNode( str( surfd ), str( data_dir ), surfd_err=sys.stderr )
