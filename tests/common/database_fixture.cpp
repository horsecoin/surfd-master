...
//using namespace surf::chain::test;

uint32_t SURF_TESTING_GENESIS_TIMESTAMP = 1431700000;
...
clean_database_fixture::clean_database_fixture()
{
   try {
      ...
      auto wit_plugin = app.register_plugin< surf::witness::witness_plugin >();
      ...
      generate_block();
      db.set_hardfork( SURF_NUM_HARDFORKS );
      generate_block();
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db.head_block_time() - timestamp ).to_seconds() < SURF_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      transfer( SURF_INIT_MINER_NAME, account_name, amount );
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, SURF_SYMBOL );
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   {
      itr--;
      ops.push_back( fc::raw::unpack< surf::chain::operation >( db.get(itr->op).serialized_op ) );
   }
