...
#include <surf/chain/hardfork.hpp>
#include <surf/chain/history_object.hpp>
#include <surf/chain/surf_objects.hpp>

#include <surf/chain/util/reward.hpp>

#include <surf/plugins/debug_node/debug_node_plugin.hpp>
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = SURF_ROOT_POST_PARENT;
         ...
         tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
      ...
      auto reward_surf = db.get_dynamic_global_properties().total_reward_fund_surf;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${surf}", ("n", author.name)("surf", a.reward_surf_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db.get_account(voter.name);
         ilog( "${n} : ${surf}", ("n", voter.name)("surf", a.reward_surf_balance));
      }
      */
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      // If comments are paid out independent of order, then the last satoshi of SURF cannot be divided among them
      const auto rf = db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME );
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      {
         const auto& post_rf = db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME );
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      auto alice_vshares = util::evaluate_reward_curve( db.get_comment( "alice", string( "test" ) ).net_rshares.value,
         db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME ).author_reward_curve,
         db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * SURF_BLOCK_INTERVAL ) / SURF_RECENT_RSHARES_DECAY_RATE.to_seconds();
         const auto& post_rf = db.get< reward_fund_object, by_name >( SURF_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - SURF_BLOCK_INTERVAL, true );

      auto reward_surf = db.get_dynamic_global_properties().total_reward_fund_surf + ASSET( "1.667 TESTS" );
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_surf.amount.value ) / total_rshares2 ).to_uint64(), SURF_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, SURF_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, SURF_SYMBOL) ) * db.get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db.get_dynamic_global_properties().total_reward_fund_surf == reward_surf - bob_comment_payout );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db.head_block_time().sec_since_epoch() + SURF_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db.get_comment( "bob", string( "test" ) ).cashout_time - ( SURF_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_surf = db.get_dynamic_global_properties().total_reward_fund_surf + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db.get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_surf.amount.value ) / total_rshares2 ).to_uint64(), SURF_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, SURF_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, SURF_SYMBOL ) ) * db.get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), SURF_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db.get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), SURF_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db.get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), SURF_SYMBOL );
      ...
      BOOST_REQUIRE( db.get_dynamic_global_properties().total_reward_fund_surf.amount.value == reward_surf.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - ( SURF_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_surf = db.get_dynamic_global_properties().total_reward_fund_surf + ASSET( "2.000 TESTS" );
      ...
      u256 rf( reward_surf.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), SURF_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, SURF_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, SURF_SYMBOL ) ) * db.get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), SURF_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db.get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), SURF_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db.get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), SURF_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db.get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), SURF_SYMBOL );
      ...
      BOOST_REQUIRE( ( db.get_dynamic_global_properties().total_reward_fund_surf + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_surf.amount.value );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      vote_op.weight = SURF_100_PERCENT;
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = SURF_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      tx.sign( bob_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( SURF_BLOCK_INTERVAL ), true );

      auto gpo = db.get_dynamic_global_properties();
      uint128_t reward_surf = gpo.total_reward_fund_surf.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_surf * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_surf -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), SURF_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "alice", string( "test" ).id, db.get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), SURF_SYMBOL );
      reward_surf += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_surf * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_surf -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), SURF_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), SURF_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "bob", string( "test" ).id, db.get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), SURF_SYMBOL );
      reward_surf += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_surf * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_surf -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db.get_comment( "dave", string( "test" ).id, db.get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), SURF_SYMBOL );
      reward_surf += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;
      ...
      auto alice_total_sbd = alice_starting_sbd + asset( alice_pays_alice_sbd + bob_pays_alice_sbd + dave_pays_alice_sbd, SURF_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, SURF_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db.get_account( "alice" ).vesting_shares.amount.value == alice_total_vesting.amount.value );

      auto bob_total_sbd = bob_starting_sbd + asset( bob_pays_bob_sbd + dave_pays_bob_sbd, SURF_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, SURF_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db.get_account( "bob" ).vesting_shares.amount.value == bob_total_vesting.amount.value );

      auto sam_total_sbd = sam_starting_sbd + asset( dave_pays_sam_sbd, SURF_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, SURF_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db.get_account( "sam" ).vesting_shares.amount.value == sam_total_vesting.amount.value );

      auto dave_total_sbd = dave_starting_sbd + asset( dave_pays_dave_sbd, SURF_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, SURF_SYMBOL ) * gpo.get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db.head_block_time() + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( SURF_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < SURF_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db.head_block_time() + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == SURF_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db.head_block_time() + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_vesting_withdraw_operation >();
         gpo = db.get_dynamic_global_properties();

         BOOST_REQUIRE( db.get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = SURF_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = SURF_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * SURF_1_PERCENT * 20 ) / SURF_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * SURF_1_PERCENT * 50 ) / SURF_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * SURF_1_PERCENT * 30 ) / SURF_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = SURF_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      ...
      generate_blocks( db.get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * SURF_1_PERCENT * 50 ) / SURF_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( surf_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing SURF Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( SURF_MIN_CONTENT_REWARD, asset( ( SURF_CONTENT_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) )
         + std::max( SURF_MIN_CURATE_REWARD, asset( ( SURF_CURATE_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
      auto witness_pay = std::max( SURF_MIN_PRODUCER_REWARD, asset( ( SURF_PRODUCER_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_surf = asset( 0, SURF_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db.get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_surf += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_surf );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_surf;
      new_rewards += gpo.total_reward_fund_surf;
      new_vesting_surf += gpo.total_vesting_fund_surf;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_surf.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_surf.amount.value == new_vesting_surf.amount.value );
      ...
      while( db.head_block_num() < SURF_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( SURF_MIN_CONTENT_REWARD, asset( ( SURF_CONTENT_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) )
            + std::max( SURF_MIN_CURATE_REWARD, asset( ( SURF_CURATE_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
         witness_pay = std::max( SURF_MIN_PRODUCER_REWARD, asset( ( SURF_PRODUCER_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
         new_vesting_surf = asset( 0, SURF_SYMBOL );
         ...
         if ( db.get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, SURF_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_surf;
         new_rewards += gpo.total_reward_fund_surf;
         new_vesting_surf += gpo.total_vesting_fund_surf;
      ...
      while( db.head_block_num() < SURF_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
         ...
         witness_pay = std::max( SURF_MIN_PRODUCER_REWARD, asset( ( SURF_PRODUCER_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_surf = asset( ( witness_pay + new_rewards ).amount * 9, SURF_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < SURF_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
         ...
         witness_pay = std::max( SURF_MIN_PRODUCER_REWARD, asset( ( SURF_PRODUCER_APR * gpo.virtual_supply.amount ) / ( SURF_BLOCKS_PER_YEAR * 100 ), SURF_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_surf = asset( ( witness_pay + new_rewards ).amount * 9, SURF_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_surf;
         new_rewards += gpo.total_reward_fund_surf;
         new_vesting_surf += gpo.total_vesting_fund_surf;
      ...
      vesting_surf = gpo.total_vesting_fund_surf;
      reward_surf = gpo.total_reward_fund_surf;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_surf.amount.value,
         ( vesting_surf.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / SURF_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / SURF_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_surf.amount.value,
         reward_surf.amount.value + virtual_supply.amount.value / 10 / SURF_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / SURF_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / SURF_BLOCKS_PER_YEAR ) * SURF_1_PERCENT ) / SURF_100_PERCENT, SURF_SYMBOL ) * ( vesting_shares / vesting_surf ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test1" ) ).reward_weight == SURF_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db.head_block_time() + SURF_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( SURF_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test2" ) ).reward_weight == SURF_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test3" ) ).reward_weight == SURF_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test4" ) ).reward_weight == SURF_100_PERCENT );
      ...
      BOOST_REQUIRE( db.get_comment( "alice", string( "test5" ) ).reward_weight == SURF_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = SURF_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      {
         db.modify( db.get_account( SURF_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_surf_balance = ASSET( "1.000 TESTS" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_surf = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_vesting_surf += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_surf_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_vesting_surf == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_surf_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( SURF_NULL_ACCOUNT ).reward_vesting_surf == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "6.000 TESTS" ) );
   }
   FC_LOG_AND_RETHROW()
}
