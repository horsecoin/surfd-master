...
#include <surf/witness/witness_objects.hpp>
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      const account_object& init = db.get_account( SURF_INIT_MINER_NAME );
      ...
      op.fee = asset( 100, SURF_SYMBOL );
      op.new_account_name = "alice";
      op.creator = SURF_INIT_MINER_NAME;
      ...
      auto vests = gpo.total_vesting_fund_surf;
      ...
      /// because init_witness has created vesting shares and blocks have been produced, 100 SURF is worth less than 100 vesting shares due to rounding
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db.get_account( SURF_INIT_MINER_NAME ).balance.amount + 1, SURF_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      tx.sign( init_account_priv_key, db.get_chain_id() );
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      tx.sign( active_key, db.get_chain_id() );
      tx.sign( active_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      tx.sign( active_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / SURF_BLOCK_INTERVAL );
      ...
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.sign( bob_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      tx.sign( bob_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db.head_block_time() + fc::seconds( SURF_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / SURF_BLOCK_INTERVAL + 1 );
...
//      const auto& mod_alice_comment = db.get_comment( "alice", string( "lorem" ) );
      ...
      {
         o.total_reward_shares2 = surf::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / SURF_BLOCK_INTERVAL );
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      tx.set_expiration( db.head_block_time() + SURF_MIN_TRANSACTION_EXPIRATION_LIMIT );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::assert_exception );


      BOOST_TEST_MESSAGE( "--- Test success deleting a comment with negative rshares" );

      generate_block();
      vote.weight = -1 * SURF_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      ...
      generate_blocks( SURF_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / SURF_BLOCK_INTERVAL );
      ...
      generate_blocks( SURF_CASHOUT_WINDOW_SECONDS / SURF_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db.get_dynamic_global_properties().vote_power_reserve_rate * SURF_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / SURF_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / SURF_100_PERCENT );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db.head_block_time() + SURF_MIN_VOTE_INTERVAL_SEC );
         ...
         BOOST_REQUIRE( db.get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * SURF_100_PERCENT / ( 2 * max_vote_denom * SURF_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db.get_account( "alice" ).voting_power ) / SURF_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db.head_block_time() + fc::seconds( ( SURF_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         uint128_t new_cashout_time = db.head_block_time().sec_since_epoch() + SURF_CASHOUT_WINDOW_SECONDS;

         BOOST_REQUIRE( new_bob.voting_power == SURF_100_PERCENT - ( ( SURF_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / SURF_100_PERCENT );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * SURF_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         new_cashout_time = db.head_block_time().sec_since_epoch() + SURF_CASHOUT_WINDOW_SECONDS;
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( SURF_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / SURF_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( new_sam.voting_power == SURF_100_PERCENT - ( ( SURF_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + SURF_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (SURF_100_PERCENT * ( db.head_block_time() - db.get_account( "alice").last_vote_time ).to_seconds() ) / SURF_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / SURF_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( db.get_comment( "alice", string( "foo" ) ).cashout_time == db.get_comment( "alice", string( "foo" ) ).created + SURF_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( SURF_1_PERCENT * 25 * ( new_alice.voting_power ) / SURF_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         auto alice_voting_power = new_alice.voting_power - used_power;

         op.voter = "alice";
         op.weight = SURF_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / SURF_100_PERCENT ).to_uint64();
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( SURF_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / SURF_100_PERCENT;
         used_power = ( used_power + max_vote_denom - 1 ) / max_vote_denom;
         alice_voting_power -= used_power;

         op.weight = SURF_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - SURF_UPVOTE_LOCKOUT ).sec_since_epoch() + SURF_BLOCK_INTERVAL ), true );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_surf.amount, SURF_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_surf.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, SURF_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      SURF_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( SURF_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( SURF_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + SURF_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_surf;
            gpo.total_vesting_fund_surf = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = asset( SURF_MIN_ACCOUNT_CREATION_FEE + 10, SURF_SYMBOL);
      op.props.maximum_block_size = SURF_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == SURF_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == SURF_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == SURF_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = SURF_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == SURF_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = SURF_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE( db.get_witness( SURF_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db.get_witness( SURF_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
//      acc_create.json_metadata = "";
      ...
      generate_blocks( db.head_block_time() + SURF_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db.head_block_time() + SURF_OWNER_UPDATE_LIMIT + fc::seconds( SURF_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db.head_block_time() + SURF_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - SURF_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db.head_block_time() + ( SURF_OWNER_AUTH_RECOVERY_PERIOD - SURF_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      SURF_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      SURF_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      SURF_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db.head_block_time() + SURF_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( SURF_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      SURF_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      SURF_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.from = "alice";
      op.to = "bob";
      op.surf_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      ...
      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SURF" );
      op.surf_amount.symbol = SURF_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      SURF_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when surf == 0" );
      op.fee.symbol = SURF_SYMBOL;
      op.surf_amount.amount = 0;
      SURF_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when surf < 0" );
      op.surf_amount.amount = -100;
      SURF_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee < 0" );
      op.surf_amount.amount = 1000;
      op.fee.amount = -100;
      SURF_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_surf_balance = alice.balance - op.surf_amount - op.fee;
      auto bob_surf_balance = bob.balance;
      auto sam_surf_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.surf_balance == op.surf_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_surf_balance );
      BOOST_REQUIRE( bob.balance == bob_surf_balance );
      BOOST_REQUIRE( sam.balance == sam_surf_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.surf_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.surf_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db.head_block_time() + SURF_BLOCK_INTERVAL );
      tx.sign( bob_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      ...
      SURF_REQUIRE_THROW( db.get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + SURF_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.surf_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db.head_block_time() + SURF_BLOCK_INTERVAL;
      et_op.escrow_expiration = db.head_block_time() + 2 * SURF_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
      BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
      BOOST_REQUIRE( escrow.surf_balance == et_op.surf_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when surf < 0" );
      op.surf_amount.amount = -1;
      SURF_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when surf == 0 " );
      SURF_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when surf is not surf symbol" );
      op.surf_amount = ASSET( "1.000 TBD" );
      SURF_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.surf_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db.get_escrow( op.from, op.escrow_id ).surf_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( op.from, op.escrow_id ).surf_balance == ASSET( "0.800 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "bob" ).balance == ASSET( "0.100 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- failure when releasing more surf than available" );
      op.surf_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less surf than available" );
      op.surf_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).surf_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).surf_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).surf_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.surf_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      tx.sign( sam_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );

      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      SURF_REQUIRE_THROW( db.get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).surf_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).surf_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.surf_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db.head_block_time() + SURF_OWNER_AUTH_RECOVERY_PERIOD );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * SURF_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db.head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * SURF_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "Testing wrong SURF symbol" );
      op.reward_surf = ASSET( "1.000 WRONG" );
      SURF_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
//      BOOST_REQUIRE( delegation->min_delegation_time == db.head_block_time() + SURF_CREATE_ACCOUNT_DELEGATION_TIME );
//      auto del_amt = delegation->vesting_shares;
//      auto exp_time = delegation->min_delegation_time;
//
//      generate_block();
//
//      BOOST_TEST_MESSAGE( "--- Test success using only SURF to reach target delegation." );
//
//      tx.clear();
//      op.fee=asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * SURF_CREATE_ACCOUNT_WITH_SURF_MODIFIER * SURF_CREATE_ACCOUNT_DELEGATION_RATIO, SURF_SYMBOL );
...
//      fund( "alice" , asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * SURF_CREATE_ACCOUNT_WITH_SURF_MODIFIER * SURF_CREATE_ACCOUNT_DELEGATION_RATIO , SURF_SYMBOL ));
//      SURF_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
//
//      validate_database();
//
...
//         {
//            a.reward_surf_balance = ASSET( "10.000 TESTS" );
//            a.reward_vesting_balance = ASSET( "10.000000 VESTS" );
//            a.reward_vesting_surf = ASSET( "10.000 TESTS" );
//         });
//
//         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
//         {
...
//            gpo.pending_rewarded_vesting_surf += ASSET( "10.000 TESTS" );
//         });
//      });
//
//      generate_block();
//      validate_database();
//
//      auto alice_surf = db.get_account( "alice" ).balance;
//      auto alice_vests = db.get_account( "alice" ).vesting_shares;
//
//
//      BOOST_TEST_MESSAGE( "--- Attempting to claim more SURF than exists in the reward balance." );
//
//      claim_reward_balance_operation op;
//      signed_transaction tx;
//
//      op.account = "alice";
//      op.reward_surf = ASSET( "20.000 TESTS" );
//      op.reward_vests = ASSET( "0.000000 VESTS" );
//
//      tx.operations.push_back( op );
//      tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
...
//      db.push_transaction( tx, 0 );
//
//      BOOST_REQUIRE( db.get_account( "alice" ).balance == alice_surf + op.reward_surf );
//      BOOST_REQUIRE( db.get_account( "alice" ).reward_surf_balance == ASSET( "10.000 TESTS" ) );
...
//      BOOST_REQUIRE( db.get_account( "alice" ).reward_vesting_surf == ASSET( "5.000 TESTS" ) );
//      validate_database();
//
...
//
//      op.reward_surf = ASSET( "10.000 TESTS" );
//      tx.clear();
//      tx.operations.push_back( op );
//      tx.sign( alice_private_key, db.get_chain_id() );
//      db.push_transaction( tx, 0 );
//
//      BOOST_REQUIRE( db.get_account( "alice" ).balance == alice_surf + op.reward_surf );
//      BOOST_REQUIRE( db.get_account( "alice" ).reward_surf_balance == ASSET( "0.000 TESTS" ) );
//      BOOST_REQUIRE( db.get_account( "alice" ).vesting_shares == alice_vests + op.reward_vests );
//      BOOST_REQUIRE( db.get_account( "alice" ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
//      BOOST_REQUIRE( db.get_account( "alice" ).reward_vesting_surf == ASSET( "0.000 TESTS" ) );
//      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / SURF_100_PERCENT );
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / SURF_100_PERCENT );
      ...
      tx.set_expiration( db.head_block_time() + SURF_MAX_TIME_UNTIL_EXPIRATION );
      tx.sign( sam_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( exp_obj->expiration == db.head_block_time() + SURF_CASHOUT_WINDOW_SECONDS );
      ...
      generate_blocks( exp_obj->expiration + SURF_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), SURF_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      SURF_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), SURF_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), SURF_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      SURF_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", SURF_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", SURF_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      SURF_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", SURF_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", SURF_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      SURF_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
//      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 TBD" ) ) );
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), SURF_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( SURF_INIT_MINER_NAME + fc::to_string( i ) ), SURF_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), SURF_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * SURF_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * SURF_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * SURF_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.sign( alice_private_key, db.get_chain_id() );
      SURF_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - SURF_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_surf;
            gpo.total_reward_fund_surf = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_surf;
         });
      });
      ...
      BOOST_REQUIRE( db.get_account( "bob" ).reward_surf_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "bob" ).reward_vesting_surf.amount + db.get_account( "sam" ).reward_vesting_surf.amount == db.get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db.get_account( "alice" ).reward_vesting_surf.amount ) == db.get_account( "bob" ).reward_vesting_surf.amount + 2 );
      BOOST_REQUIRE( ( db.get_account( "alice" ).reward_vesting_surf.amount ) * 2 == db.get_account( "sam" ).reward_vesting_surf.amount + 3 );
   }
   FC_LOG_AND_RETHROW()
}
